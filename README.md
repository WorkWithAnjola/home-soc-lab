...
- Installed VirtualBox, created a Host-only network for lab isolation
...
- Resource-constrained VMs (6GB RAM) can cause install processes to fail unpredictably — patience and systematic log-reading (`/var/log/wazuh-install.log`) were essential for diagnosing each failure
- Wazuh's manager + agent cannot coexist on the same host in a standard install — a dedicated agent host is needed for full endpoint monitoring

## Next Steps

- [ ] Build a dedicated Windows or Linux victim VM with a Wazuh agent installed
- [ ] Simulate additional attack scenarios (brute-force SSH, malware indicators)
- [ ] Document detection → triage → response workflow for each scenario
- [ ] Explore custom detection rules in Wazuh

## Screenshots

*(Add screenshots here as the lab progresses — dashboard overview, alert details, attack scenarios)*

---

Built as a learning project for hands-on security operations experience.
